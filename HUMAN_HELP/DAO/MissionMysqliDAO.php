...
class MissionMySqliDAO
{
    ...
    public static function searchMissionById($getIdMission)
    {
        ...
        $stmt = $db->prepare($query);
        $stmt->bind_param("i", $getIdMission);
        $stmt->execute();       
        $rs = $stmt->get_result();
        $mission = $rs->fetch_array(MYSQLI_ASSOC);
...
/**************** CHERCHE TOUTES LES MISSIONS PAR TYPE D'ACTIVITE *******/
    public function searchMissionByTypActivite($id_type_activite){
        ...
        $query = "SELECT * FROM mission WHERE id_type_activite = ?";

        $stmt = $db->prepare($query);
        $stmt->bind_param("i", $id_type_activite);
        $stmt->execute();       
        $rs = $stmt->get_result();
        $missions = $rs->fetch_all(MYSQLI_ASSOC);
        ...
        foreach ($missions as $mission) 
        {
            ...
            $newMission->setIdMission($mission['id_mission'])
                       ...
                       ->setIdEtablissement($mission['id_etablissement'])
                       ->setIdTypeActivite($mission['id_type_activite']);
            $MissionsByTypeActivite[$i] = $newMission;
            $i++;
        }                      
        return $MissionsByTypeActivite;
    }
/**************** CHERCHE TOUTES LES MISSIONS PAR PAYS *******/
    public function searchMissionByPays($id_pays){
        $db = connexion();
        
        $query = "SELECT * FROM mission WHERE id_pays = ?";
        ...
        foreach ($missions as $mission) 
        {
            ...
            $newMission->setIdMission($mission['id_mission'])
                       ...
                       ->setIdEtablissement($mission['id_etablissement'])
                       ->setIdTypeActivite($mission['id_type_activite']);
            $MissionsByPays[$i] = $newMission;
            $i++;
        }                      
        return $MissionsByPays;
    }
}
